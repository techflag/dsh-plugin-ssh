...
interface Document { path: string; text: string; saved: string; version: string; busy: boolean; size: number; editable: boolean; truncated: boolean }
...
export function installEditor(api: <T>(op:string,data:unknown)=>Promise<T>, getId:()=>string) {
  ...
  function refresh(){const d=documents.get(getId());panel.hidden=!d;layout();if(!d)return;input.value=d.text;input.disabled=d.busy;input.readOnly=!d.editable;panel.classList.toggle('preview-only',!d.editable);label.textContent=d.path+(d.editable&&d.text!==d.saved?' ●':'');save.hidden=!d.editable;save.disabled=d.busy||d.text===d.saved;status.textContent=d.editable?`可编辑 · ${formatBytes(d.size)}`:`只读预览 · ${d.truncated?'显示末尾约 512 KB / ':''}共 ${formatBytes(d.size)}，下载可查看完整文件`}
  ...
  return {refresh,canClose,async open(id:string,path:string){if(!(await canClose(id)))return;const current=++generation;try{const result=await api<{text:string;version:string;size:number;editable:boolean;truncated:boolean}>('read-text',{id,path});if(current!==generation)return;documents.set(id,{path,text:result.text,saved:result.text,version:result.version,busy:false,size:result.size,editable:result.editable,truncated:result.truncated});if(id===getId())refresh()}catch(error){throw error}}}
}
