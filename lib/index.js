...
var SshService = class {
	...
	async upload(id, path, source, onProgress, overwrite = false) {
		...
		let handle;
		try {
			handle = await new Promise((resolve, reject) => s.sftp.open(temporary, "w", { mode: 384 }, (error, value) => error ? reject(error) : resolve(value)));
		} catch {
		...
		try {
			if (!overwrite) {
				if (await new Promise((resolve) => s.sftp.lstat(target, (error) => resolve(!error)))) throw new Error("目标文件已存在");
				await new Promise((resolve, reject) => s.sftp.rename(temporary, target, (error) => error ? reject(error) : resolve()));
				return;
			}
			let mode = 384;
			try {
	...
	async readText(id, path) {
		...
		const chunks = [];
		let bytes = 0;
		await pipeline(s.sftp.createReadStream(target, start ? { start } : void 0), new Writable({ write(data, _encoding, done) {
			bytes += data.length;
			if (bytes > 524292) {
				done(/* @__PURE__ */ new Error("文本预览超过上限"));
				return;
			}
		...
		try {
			let decoded;
			for (let offset = 0; offset <= Math.min(4, content.length); offset++) try {
				decoded = new TextDecoder("utf-8", {
					fatal: true,
					ignoreBOM: true
				}).decode(content.subarray(offset));
				break;
			} catch {}
			if (decoded === void 0) throw new Error();
			text = decoded;
		} catch {
